...
const SHOPIFY_STORE_PERMANENT_DOMAIN = 'streim.myshopify.com';
const SHOPIFY_STOREFRONT_URL = `https://${SHOPIFY_STORE_PERMANENT_DOMAIN}/api/${SHOPIFY_API_VERSION}/graphql.json`;
const SHOPIFY_STOREFRONT_TOKEN = 'e061e27584f2d9875918e5e88ff885f9';
...
export async function storefrontApiRequest(query: string, variables: Record<string, unknown> = {}) {
  ...
  if (response.status === 402) {
    toast.error("Shopify: Betaling vereist", {
      description: "Je Shopify store heeft een actief abonnement nodig.",
    });
    return;
  }
...
export async function syncShopifyCart(cartId: string): Promise<boolean> {
  ...
  if (!data) return true; // preserve on API error
